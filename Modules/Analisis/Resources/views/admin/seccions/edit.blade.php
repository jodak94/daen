...
@section('content-header')
    <h1>
        {{ trans('analisis::seccions.title.edit seccion') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.analisis.seccion.index') }}">{{ trans('analisis::seccions.title.seccions') }}</a></li>
        <li class="active">{{ trans('analisis::seccions.title.edit seccion') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.analisis.seccion.update', $seccion->id], 'method' => 'put']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('analisis::admin.seccions.partials.edit-fields', ['lang' => $locale])
                        </div>
                    @endforeach
